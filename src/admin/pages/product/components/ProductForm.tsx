...
const ProductForm = ({ productTitle, productSubtitle, product,isPending, onSubmit }: Props) => { 
  ...
  const addTag = () => {
    if(!tagsRef.current) return
     ...
     return tagsRef.current.value = ''
  }; 
  const removeTag = (tagToRemove: string) => {

    const tagRemoved = getValues('tags').filter((tag:string) => tag !== tagToRemove);

    setValue('tags',tagRemoved)
  };
  ...
  const removeSize = (sizeToRemove: Size) => { 
    const filteredSize = getValues('sizes').filter(size => size !== sizeToRemove);
    setValue('sizes', Array.from(filteredSize)); 
            ...
            <div className="bg-white rounded-xl shadow-lg border border-slate-200 p-6">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  />
                  {errors.title && (
                    <p className="font-montserrat font-light text-red-400">
